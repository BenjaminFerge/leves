...
/// enabling/disabling respective reactor framework notifications.
{
	      ...
	      _fifoOut(BUFFER_SIZE, true)
	{
		...
		_reactor.addEventHandler(
		    _socket,
		    NObserver<EchoServiceHandler, ReadableNotification>(
			*this, &EchoServiceHandler::onSocketReadable));
		_reactor.addEventHandler(
		    _socket,
		    NObserver<EchoServiceHandler, ShutdownNotification>(
			*this, &EchoServiceHandler::onSocketShutdown));

		_fifoOut.readable +=
		    delegate(this, &EchoServiceHandler::onFIFOOutReadable);
		_fifoIn.writable +=
		    delegate(this, &EchoServiceHandler::onFIFOInWritable);
	}
	...
	~EchoServiceHandler()
	{
		...
		_reactor.removeEventHandler(
		    _socket,
		    NObserver<EchoServiceHandler, WritableNotification>(
...
///
/// To test the EchoServer you can use any telnet client (telnet localhost
/// 9977).
{
    public:
	...
	int main(const std::vector<std::string>& args)
	{
		if (_helpRequested) {
			...
			SocketAcceptor<EchoServiceHandler> acceptor(svs,
								    reactor);
