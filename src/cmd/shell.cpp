...
std::unique_ptr<cmd::Command> cmd::Shell::interpret(std::string in)
{
    ...
    case Shell_cmd::delete_stream: {
        if (argv.size() != 1) {
            return std::make_unique<cmd::Invalid>(Delete_stream::usage());
        }
        int id;
        try {
            id = std::stoi(argv[0]);
        ...
        auto req = new cmd::Delete_stream_req(id);
        return std::make_unique<cmd::Delete_stream>(handler_, *req);
    }
    case Shell_cmd::push: {
        if (argv.size() != 4) {
...
cmd::Shell::tokens(std::string in)
{
    ...
    if (first == "delete_stream")
        c = cmd::Shell::Shell_cmd::delete_stream;
